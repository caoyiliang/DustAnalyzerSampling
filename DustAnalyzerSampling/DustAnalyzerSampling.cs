...
{
    public class DustAnalyzerSampling : IDustAnalyzerSampling
    {
        private static readonly ILogger _logger = Logs.LogFactory.GetLogger<DustAnalyzerSampling>();
        ...
        public DustAnalyzerSampling(SerialPort serialPort, int defaultTimeout = 5000)
        {
        ...
        public DustAnalyzerSampling(ICrowPort crowPort)
        {
        ...
        {
            _logger.Trace($"DustAnalyzerSampling Rec:<-- {StringByteUtils.BytesToString(data)}");
            await Task.CompletedTask;
        }

        private async Task CrowPort_OnSentData(byte[] data)
        {
            _logger.Trace($"DustAnalyzerSampling Send:--> {StringByteUtils.BytesToString(data)}");
            await Task.CompletedTask;
        }
        ...
        public async Task<Dictionary<string, string>?> Read(string addr, int tryCount = 0, CancellationToken cancelToken = default)
        {
            ...
            var b = new BlockList();
            b.Add(new PDustAnalyzerSampling());
            Func<Task<PDustAnalyzerSampling>> func = () => _modBusMaster.GetAsync<PDustAnalyzerSampling>(addr, b);
